...
//! \brief Creates a quaternion with no rotation.
__forceinline quatf_t quatf_identity()
{
	return (quatf_t){ .x = 0.0f, .y = 0.0f, .z = 0.0f, .w = 1.0f };
}

//! \brief Combines the rotation of two quaternions -- a and b -- into a new quaternion.
...
//! \brief Computes the inverse of a normalized quaternion.
...
//! \brief Rotates a vector by a quaterion.
...
//! \brief Converts a quaternion to radians:
//!
//! Uses roll, yaw, pitch to create representation of 3 angles
vec3f_t quatf_to_eulers(quatf_t q);

//! \brief Converts roll, yaw, pitch in radians to a quaternion.
